    ...
    <div class="row mb-2">
      ...
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          ...
          <li class="breadcrumb-item active"><?php echo lang('customers') ?></li>
        </ol>
      </div>
    </div>
  </div><!-- /.container-fluid -->
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header d-flex p-0">
                <h3 class="card-title p-3"><?php echo lang('customers') ?></h3>
                <div class="ml-auto p-2">
                    <?php if (hasPermissions('customers_add')): ?>
                      <a href="<?php echo url('customers/add') ?>" class="btn btn-primary btn-sm"><span class="pr-1"><i class="fa fa-plus"></i></span> <?php echo lang('new_customer') ?></a>
                    <?php endif ?>
                </div>
              </div>
                  ...
                  <?php foreach ($customers as $row): ?>
                    <tr>
                      ...
                      <td width="50" class="text-center">
                        <img src="<?php echo customerProfile($row->id) ?>" width="40" height="40" alt="" class="img-avtar">
                      ...
                      <td>
                        <?php if (hasPermissions('customers_edit')): ?>
                          <a href="<?php echo url('customers/edit/'.$row->id) ?>" class="btn btn-sm btn-primary" title="<?php echo lang('edit_customer') ?>" data-toggle="tooltip"><i class="fas fa-edit"></i></a>
                        <?php endif ?>
                        <?php if (hasPermissions('customers_view')): ?>
                          <a href="<?php echo url('customers/view/'.$row->id) ?>" class="btn btn-sm btn-info" title="<?php echo lang('view_customer') ?>" data-toggle="tooltip"><i class="fa fa-eye"></i></a>
                        <?php endif ?>
                        <?php if (hasPermissions('customers_delete')): ?>
                          <?php if ($row->id!=1 && logged('id')!=$row->id): ?>
                            <a href="<?php echo url('customers/delete/'.$row->id) ?>" class="btn btn-sm btn-danger" onclick="return confirm('Do you really want to delete this user ?')" title="<?php echo lang('delete_customer') ?>" data-toggle="tooltip"><i class="fa fa-trash"></i></a>
                          <?php else: ?>
...
window.updateUserStatus = (id, status) => {
  $.get( '<?php echo url('customers/change_status') ?>/'+id, {
    status: status
  }, (data, status) => {
    if (data=='done') {
      // code
